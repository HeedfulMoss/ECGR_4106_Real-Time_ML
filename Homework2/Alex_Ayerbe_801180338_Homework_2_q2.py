...
#------------------------------------------------------------------------------
# VGG-style models with parameter count close to AlexNet
#------------------------------------------------------------------------------
class VGGNet(nn.Module):
    def __init__(self, num_classes=10):
        super(VGGNet, self).__init__()
        self.features = nn.Sequential(
            # Block 1: from 32x32 -> 16x16
            ...
            # Block 3: preserve spatial resolution, then use adaptive pooling
        ...
        # Force the conv output to a fixed 6x6 spatial size
        self.avgpool = nn.AdaptiveAvgPool2d((6, 6))
        
        # Classifier (same as in AlexNet) producing most of the parameters
...
class VGGNetDropout(nn.Module):
    def __init__(self, num_classes=10):
        super(VGGNetDropout, self).__init__()
        self.features = nn.Sequential(
            # Block 1: from 32x32 -> 16x16
            nn.Conv2d(3, 64, kernel_size=3, padding=1),  # 3->64
            nn.ReLU(inplace=True),
            ...
            # Block 3: preserving spatial dimensions before adaptive pooling
            nn.Conv2d(128, 256, kernel_size=3, padding=1),  # 128->256
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
...
def compare_models(model_classes, optim_configs, train_loader, validation_loader, class_labels, num_epochs=10, lr=0.01, momentum=0.9, show_figures=False, save_figures=True, dataset_name="UnknownDataset", save_txt=True):
    ...
    model_results_list = []
    header = f"model_comparison_{dataset_name}"
...
# ---- Call the function for CIFAR-10 ----
compare_models([VGGNet, VGGNetDropout], optim_configs, train_loader_10, validation_loader_10, classes_cifar10, num_epochs=10, dataset_name="cifar10")

# ---- Call the function for CIFAR-100 ----
compare_models([VGGNet, VGGNetDropout], optim_configs, train_loader_100, validation_loader_100, classes_cifar100, num_epochs=10, dataset_name="cifar100")
